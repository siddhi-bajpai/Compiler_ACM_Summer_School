...
gen_decl(string name, Type_Info *type_info)
{
	if (!type_info->is_tensor_type())
	{
		...
		genf<< mangle_name(name);
		genf<<"(";
		genf<<type_info->get_number_of_dimensions();
		genf<<",";
		genf<<type_info->get_size_of_first_dim();

		if(type_info->get_number_of_dimensions()==1){
		genf<<")";
		}
		else{
		genf<<",";
		genf<<type_info->get_size_of_second_dim();
		genf<<")";
		}
		genf<<";\n";
